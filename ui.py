...
def searhMovieInRating(tree, B_0, typeName, count, rating, vote):
    """
    从排行榜中搜索符合条件的影片信息
    :param root: root
    :param w: 窗口宽度
    ...
    movieObject = getMovieInRankingList();
    jsonMovieData = json.loads(movieObject.movieData)
    for subMovieData in jsonMovieData:
        if(subMovieData['title'] == typeName):
            clearTree(tree)  # 清空表格
            list = movieObject.getUrlDataInRankingList(subMovieData['type'], count, rating, vote)  # 返回符合条件的电影信息
            addTree(tree, list) # 将数据添加到tree中
            break
...
def ui_process():
    ...
    # 将影片类型输入到下拉列表框中
    movieObject = getMovieInRankingList();
    jsonMovieData = json.loads(movieObject.movieData) #json数据
    movieList = []
    ...
    B_0.configure(command=lambda : thread_it(searhMovieInRating, treeview, B_0, C_type.get(), T_count.get(), T_rating.get(), T_vote.get()))
